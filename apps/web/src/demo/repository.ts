import type { Debt, DemoData, QueuedMutation, Scenario, ScenarioAdjustment, Transaction } from '../domain/models';
...
export class DemoRepository {
  private recalculate(data: DemoData) {
    ...
    const result = calculatePlan(data.baseline, data.debts, data.activeScenario, stamp, data.currency);
    data.plan = result.plan; data.projection = result.projection; data.updatedAt = stamp;
    data.explanation.headline = data.plan.action.title; data.explanation.generated_at = stamp;
    return data;
  }
  async data(error = false, _offline = false) {
    ...
    if (value && !value.baseline) {
      const migrated=createDemo(value.scenario); migrated.debts=value.debts; migrated.transactions=value.transactions;
      value=this.recalculate(migrated); await setDemo(value);
    }
    if (!value) {
      value = createDemo();
      await setDemo(value);
  ...
  async submitOnboarding(draft: import('../domain/models').OnboardingDraft) {
    ...
    data.baseline = { ...data.baseline, availableNow:draft.availableNow, minimumBuffer:draft.minimumReserve,
      nextIncomeDate:draft.incomes.find(x=>x.confirmed)?.date ?? data.baseline.nextIncomeDate,
      incomes:draft.incomes.map(x=>({id:x.id,name:x.name,amount:x.amount,date:x.date,confirmed:x.confirmed})),
      expenses:draft.expenses.map(x=>({id:x.id,name:x.name,amount:x.amount,date:x.date,confirmed:x.confirmed})) };
  ...
  async applyScenario(scenario?: ScenarioAdjustment) {
    const data=(await getDemo()) ?? createDemo(); data.activeScenario=scenario;
    await setDemo(this.recalculate(data)); return data;
  }
  async reset() {
  ...
  async addTransaction(input: Omit<Transaction, 'id' | 'syncStatus'>, offline: boolean) {
    ...
    if (item.kind === 'expense' || item.kind === 'debt_payment') data.baseline.availableNow -= item.amount;
    await setDemo(this.recalculate(data));
  ...
  async deleteTransaction(id: string, offline: boolean) {
    ...
    if(removed&&(removed.kind==='expense'||removed.kind==='debt_payment')) data.baseline.availableNow+=removed.amount;
    await setDemo(this.recalculate(data));
