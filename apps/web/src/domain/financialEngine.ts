...
export const minimumDebtPayments = (debts: Debt[]) =>
  debts.reduce((sum, d) => sum + d.minimum_payment, 0);
export const monthlyInterest = (balance: number, annualRateBps: number) =>
  Math.round((balance * annualRateBps) / 120_000);
export const priorityDebt = (
  debts: Debt[],
  ...
  [...debts]
    ...
    .sort((a, b) =>
      strategy === 'snowball'
        ? a.balance - b.balance
        : strategy === 'custom'
          ? a.custom_priority - b.custom_priority
          : b.annual_rate_bps - a.annual_rate_bps || a.balance - b.balance,
    )[0];
...
export function applyScenario(
  ...
  return {
    ...
    monthlyIncome: base.monthlyIncome + scenario.extraIncome,
    minimumBuffer: scenario.reserve,
    expenses: base.expenses.map((x, i) =>
      i === 0 ? { ...x, amount: Math.max(0, x.amount + scenario.expenseChange) } : x,
    ),
  };
}
...
export function forecast(
  ...
  let reserve = Math.max(
    0,
    inputs.availableNow - dueBefore(inputs.expenses, inputs.nextIncomeDate),
  );
  let totalInterest = 0;
  ...
  for (let month = 0; month <= MAX_FORECAST_MONTHS; month++) {
    ...
    if (month > 0) {
      ...
      if (target)
        target.balance = Math.max(0, target.balance - Math.min(target.balance, acceleration));
      ...
      if (reserve >= inputs.minimumBuffer && stabilizationMonths === null)
        stabilizationMonths = month;
    }
    points.push({
      date: date.toISOString(),
      debt: balances.reduce((s, d) => s + d.balance, 0),
      reserve,
      interest: totalInterest,
    });
    if (month >= 12 && debtFreeMonths !== null && stabilizationMonths !== null) break;
  }
  const nextPayment = [...inputs.expenses]
    .filter((x) => x.confirmed && x.date >= referenceDate)
  ...
  return {
    ...
    nextPayment,
    assumptions: [
      'Подтверждённый доход поступает по плану',
      'Ставки и минимальные платежи не меняются',
      'Диапазон срока учитывает отклонение до двух месяцев',
    ],
  };
}
...
export function calculatePlan(
  inputs: FinancialInputs,
  debts: Debt[],
  scenario: ScenarioAdjustment | undefined,
  referenceDate: string,
  currency: Plan['currency'],
): { plan: Plan; projection: FinancialProjection } {
  ...
  const monthlyFree =
    current.monthlyIncome - monthlyMandatory - minimum - (scenario?.extraDebtPayment ?? 0);
  ...
  const state: Plan['state'] =
    projected < 0
      ? 'critical'
      : overdue || current.availableNow < current.minimumBuffer
        ? 'stabilization'
        : debts.some((d) => d.balance > 0)
          ? 'exit'
          : current.availableNow < current.minimumBuffer
            ? 'buffer'
            : 'growth';
  ...
  const action =
    projected < 0
      ? {
          type: 'review_expense',
          title: 'Сократите или перенесите ближайший платёж',
          amount: -projected,
        }
      : overdue
        ? {
            type: 'pay_mandatory',
            title: 'Разберите просроченный платёж',
            amount: debts.find((d) => d.overdue)?.minimum_payment ?? 0,
          }
        : current.availableNow < current.minimumBuffer
          ? {
              type: 'build_buffer',
              title: 'Защитите минимальный резерв',
              amount: current.minimumBuffer - current.availableNow,
            }
          : target
            ? {
                type: 'pay_target_debt',
                title: `Ускорьте «${target.name}»`,
                amount: Math.max(0, scenario?.extraDebtPayment ?? monthlyFree),
              }
            : {
                type: 'build_buffer',
                title: 'Продолжайте пополнять подушку',
                amount: Math.max(0, monthlyFree),
              };
  return {
    plan: {
      currency,
      state,
      snapshot: {
        available_now: current.availableNow,
        ...
        safe_to_spend: safe,
        safe_daily_amount: Math.floor(safe / days),
        monthly_free_cash_flow: monthlyFree,
        minimum_buffer_target: current.minimumBuffer,
      },
      action,
...
export const comparePlans = (base: Plan, candidate: Plan) => ({
  safeDelta: candidate.snapshot.safe_to_spend - base.snapshot.safe_to_spend,
  cashFlowDelta: candidate.snapshot.monthly_free_cash_flow - base.snapshot.monthly_free_cash_flow,
});
export const assessWorkload = (hours: number) =>
  hours <= 8 ? 'low' : hours <= 16 ? 'medium' : 'high';
export function assessPurchase(
  ...
  const conservativeIncome = Math.floor(
    (optimisticIncome * Math.max(0, Math.min(100, confidencePercent))) / 100,
  );
  return {
    conservativeIncome,
    paybackMonths: conservativeIncome > 0 ? Math.ceil(cost / conservativeIncome) : null,
    createsCashGap: cost > safeToSpend,
    risk: cost > safeToSpend ? 'high' : conservativeIncome === 0 ? 'high' : 'moderate',
  } as const;
}
