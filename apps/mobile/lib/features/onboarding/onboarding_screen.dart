...
class _State extends ConsumerState<OnboardingScreen> {
  ...
  Future<void> _next() async { error=null; if(draft.step==1||draft.step==5){final amount=parseMoney(value.text);if(amount==null){setState(()=>error='Введите корректную неотрицательную сумму');return;}if(draft.step==1)draft.available=amount;else draft.reserve=amount;value.clear();} if(draft.step<6){setState(()=>draft.step++);await _save();return;} setState(()=>busy=true);try{final api=ref.read(apiClientProvider);await api.completeOnboarding(draft.payload(),draft.idempotencyKey);ref.read(authControllerProvider.notifier).completed();if(mounted)context.go('/today');}catch(_){if(mounted)setState(()=>error='Не удалось сохранить данные. Проверьте сеть и повторите попытку.');}finally{if(mounted)setState(()=>busy=false);}}
}
